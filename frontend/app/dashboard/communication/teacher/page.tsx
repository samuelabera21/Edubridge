...
export default function TeacherCommunicationPage() {
    ...
    const loadAnnouncements = async () => {
        try {
            setLoading(true);
            const res = await fetchApi("/communication/announcements?target=TEACHERS");
            if (res.ok) {
            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-xs overflow-hidden">
                {paginatedAnnouncements.length === 0 ? (
                    <div className="p-12 text-center text-gray-500 space-y-2">
                        ...
                        <p className="font-semibold text-gray-800 text-sm">No faculty circulars found</p>
                        <p className="text-xs text-gray-400">Announcements targeted to teaching staff will appear here.</p>
                    </div>
                ) : (
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        <span className="text-xs text-gray-500 flex items-center gap-1">
                                            ...
                                            {item.author?.name || "Academic Administration"}
                                        </span>
                                    </div>
                ...
                {totalCount > 0 && (
                    <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-600">
                        <div className="flex items-center gap-3">
                            <span>
                                Showing <strong className="text-gray-900">{(currentPage - 1) * pageSize + 1}</strong> to <strong className="text-gray-900">{Math.min(currentPage * pageSize, totalCount)}</strong> of <strong className="text-gray-900">{totalCount}</strong> circulars
                            </span>
