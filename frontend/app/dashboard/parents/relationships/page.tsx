...
import { HeartHandshake, Plus, Filter, Users } from "lucide-react";
...
export default function StudentRelationshipsPage() {
    ...
    const hasCreatePermission = authData?.access.some(acc => 
        acc.role.permissions.some((p: any) => p.permission.name === "USER:MANAGE") // Assuming USER:MANAGE for parent linking
    );
    ...
    const loadData = async () => {
        try {
            ...
            // Fetch Enrollments to serve as the base for searching relationships
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 flex items-center">
                        <HeartHandshake className="w-6 h-6 mr-2 text-blue-500" />
                        Student Relationships
                    </h1>
                    <p className="text-sm text-gray-500 mt-1">
                        Link parent and guardian accounts to enrolled students.
                    </p>
                </div>
                {hasCreatePermission && (
            ...
            {enrollments.length === 0 ? (
                <EmptyState 
                    ...
                    message={`There are no students enrolled to manage relationships.`} 
                />
            ) : (
                <Card>
                                    ...
                                    {enrollments.map((enrollment) => (
                                        <tr key={enrollment.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right space-x-2">
                                                <Button variant="ghost" size="sm">Manage Links</Button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
